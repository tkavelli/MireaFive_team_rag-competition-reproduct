# Воспроизведение лучшего результата (Hit@5: 36.40%)

Этот документ содержит инструкции по воспроизведению лучшего результата на лидерборде.

## Результат

- **Hit@5**: 36.40%
- **Модель**: Qwen/Qwen3-Embedding-8B
- **Submission файл**: `outputs/submission_ch_v5_qwen3_8b.csv`
- **Дата**: 2025-11-16

---

## Требования

### Системные требования

- **GPU**: NVIDIA RTX 4090 или аналог (≥24GB VRAM)
- **RAM**: ≥32GB
- **Disk**: ≥20GB свободного места (модель ~15GB)
- **OS**: Linux (протестировано на Arch Linux)

### Софт

- Python 3.10+ (протестировано: Python 3.13)
- **CUDA 12.8** или выше (система: CUDA 13.0, PyTorch скомпилирован с 12.8)
- Git
- PyTorch 2.8.0+cu128 (устанавливается через --index-url, см. ниже)

---

## 🚀 Быстрый старт

### 1. Docker (полностью воспроизводимо)

```bash
git clone <repository-url>
cd retrieval-competition-reproduct

# Сборка образа (Ubuntu 24.04 + CUDA 12.8)
docker build -t alfa-rag-solution .

# Запуск: данные уже внутри образа, наружу отдаем только результаты и кэш HF (опционально)
docker run --gpus all \
  -v $(pwd)/outputs:/app/outputs \
  -v $(pwd)/.hf_cache:/app/.hf_cache \
  alfa-rag-solution
```

Альтернатива — использовать свои CSV вместо упакованных:

```bash
docker run --gpus all \
  -v $(pwd)/data:/app/data \
  -v $(pwd)/outputs:/app/outputs \
  -v $(pwd)/.hf_cache:/app/.hf_cache \
  alfa-rag-solution \
  bash -c "cd /app && bash run_qwen3_rrf_v2_stable.sh"
```

### 2. Локальный запуск (без Docker)

```bash
# Рекомендуем Python 3.13.7 (как в контейнере). На Arch можно через pyenv:
#   pyenv install 3.13.7 && pyenv local 3.13.7

python -m venv .venv
source .venv/bin/activate
pip3 install torch==2.8.0 torchvision==0.23.0 --index-url https://download.pytorch.org/whl/cu128
pip install -r requirements.txt

# Данные лежат в data/
bash run_qwen3_rrf_v2_stable.sh
```

**Ожидаемое время выполнения**: ~15-30 минут на RTX 4090

---

## Структура необходимых файлов

Для воспроизведения требуются следующие файлы:

```
retrieval-competition/
├── questions_clean.csv                    # Исходные вопросы
├── websites.csv                           # Корпус документов
├── requirements.txt                       # Python зависимости
├── run_qwen3_rrf_v2_stable.sh            # Скрипт запуска (точка входа)
│
├── pipelines/
│   └── retrieve.py                       # Основной retrieval pipeline
│
├── src/
│   ├── chunker.py                        # Чанкование документов
│   ├── retriever.py                      # Hybrid retriever (FAISS + BM25)
│   └── models/
│       ├── embedding_models.py           # Конфигурация моделей
│       └── qwen3_embedding_stable.py     # Qwen3 wrapper
│
└── outputs/
    ├── submission_ch_v5_qwen3_8b.csv          # Результат (создаётся после запуска)
    ├── chunks_ch_v5_qwen3_8b.csv              # Чанки, если пересчитывались
    └── faiss_index_ch_v5_qwen3_8b/            # FAISS индекс
        ├── embeddings.npy
        ├── faiss_index.bin
        ├── chunks.csv
        └── metadata.json
```

---

## Конфигурация

### Основные параметры (из `run_qwen3_rrf_v2_stable.sh`)

```bash
python pipelines/retrieve.py \
  --queries questions_clean.csv \
  --corpus websites.csv \
  --out outputs/submission_ch_v5_qwen3_8b.csv \
  --model "Qwen/Qwen3-Embedding-8B" \
  --chunk_version ch_v5 \
  --model_name qwen3_8b \
  --chunk_size 2048 \
  --overlap 200 \
  --batch_size 2 \
  --top_k 5 \
  --use_hybrid
```

### Ключевые настройки

| Параметр | Значение | Описание |
|----------|----------|----------|
| `--model` | `Qwen/Qwen3-Embedding-8B` | Embedding модель |
| `--chunk_size` | `2048` | Размер чанков в токенах |
| `--overlap` | `200` | Перекрытие между чанками |
| `--batch_size` | `2` | Batch size (для 24GB VRAM) |
| `--use_hybrid` | flag | Включает BM25 + FAISS гибрид |

### Fusion параметры (внутренние, в коде)

- `semantic_weight`: 0.7 (BM25=0.3)
- `original_weight`: 1.0
- Fusion method: Convex Combination (CC)

---

## Проверка результата

После выполнения скрипта проверьте:

```bash
# 1. Наличие submission файла
ls -lh outputs/submission_ch_v5_qwen3_8b.csv

# 2. Формат (первые 5 строк)
head -5 outputs/submission_ch_v5_qwen3_8b.csv

# Ожидаемый вывод:
# q_id,web_list
# 0,"[123, 456, 789, ...]"
# 1,"[234, 567, 890, ...]"
# ...

# 3. Количество строк (должно быть 6978: заголовок + 6977 предсказаний)
wc -l outputs/submission_ch_v5_qwen3_8b.csv
```

---

## Архитектура решения

### Pipeline Overview

```
questions_clean.csv (6977 queries)
         ↓
    [Chunker]
         ↓
websites.csv → chunks (2048 tokens, overlap 200) → ~4,242 chunks
         ↓
[Qwen3-Embedding-8B]
         ↓
   FAISS index ← embeddings
         ↓
[Hybrid Retrieval: FAISS (70%) + BM25 (30%)]
         ↓
[Convex Combination Fusion]
         ↓
   Top-5 results per query
         ↓
submission_qwen3_rrf_v2_stable.csv
```

### Ключевые компоненты

1. **SmartChunker** (`src/chunker.py`)
   - Разбивает документы на чанки фиксированного размера
   - chunk_size=2048 токенов, overlap=200 токенов
   - Использует tiktoken для подсчета токенов

2. **Qwen3EmbeddingModelStable** (`src/models/qwen3_embedding_stable.py`)
   - Wrapper для Qwen/Qwen3-Embedding-8B
   - Last token pooling (Context7 оптимизация)
   - Instruction formatting для queries
   - Flash Attention 2 отключен для стабильности

3. **HybridRetriever** (`src/retriever.py`)
   - FAISS (dense retrieval) + BM25 (sparse retrieval)
   - Convex Combination fusion: 70% semantic + 30% BM25
   - Top-k retrieval с гибридным ранжированием

---

## Troubleshooting

### CUDA Out of Memory

Если видите ошибку "CUDA out of memory":

```bash
# Уменьшите batch_size в run_qwen3_rrf_v2_stable.sh
--batch_size 1  # вместо 2
```

### Модель не загружается

```bash
# Убедитесь, что HF_HOME доступен для записи
export HF_HOME="$(pwd)/.hf_cache"
export TRANSFORMERS_CACHE="$HF_HOME"
mkdir -p "$HF_HOME"

# Повторите запуск
bash run_qwen3_rrf_v2_stable.sh
```

### Медленная работа

```bash
# Проверьте, что используется GPU
python -c "import torch; print(torch.cuda.is_available())"

# Ожидаемый вывод: True
```

---

## Примечания

1. **Детерминизм**: Результаты могут немного отличаться из-за:
   - Порядка обработки батчей
   - Версий библиотек (torch, transformers)
   - Устойчивости floating-point операций

2. **VRAM**: При batch_size=2 ожидается ~20-22GB VRAM использования

3. **Время**: ~15-30 минут на RTX 4090:
   - Чанкование: ~1-2 мин
   - Embedding генерация: ~10-20 мин
   - Retrieval: ~3-5 мин

---

## Дополнительная информация

- **Полный лог экспериментов**: `PROGRESS_LOG.md`
- **Таблица результатов**: `experiments/README.md`
- **План развития**: `REVISED_PLAN.md`
- **Описание проекта**: `CLAUDE.md`

---

## Контакты

Если возникнут вопросы по воспроизведению результата, пожалуйста:

1. Проверьте раздел **Troubleshooting** выше
2. Просмотрите логи в `PROGRESS_LOG.md`
3. Создайте issue в репозитории

---

**Последнее обновление**: 2025-11-25
